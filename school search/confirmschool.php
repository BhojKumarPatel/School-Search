...
echo("<h2>Schools for Verification</h2>");


$rsCrs=mysql_query("select s_name,s_code,s_pin,s_add,s_dist,s_state,s_email,s_yearofreg,s_classes,s_type from tempschools");
...
while($row=mysql_fetch_array($rsCrs))
{
	...
	echo($row["s_classes"]);
	echo("</td>");
	echo("<td width='200px'><a href='permanentschool.php?status=$x'>Confirm</a></td>");
	echo("<td width='200px'><a href='removeschool.php?status=$x'>Remove</a></td>");
